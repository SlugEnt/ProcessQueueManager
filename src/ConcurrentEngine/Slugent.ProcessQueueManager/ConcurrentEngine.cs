...
public class ConcurrentEngine
{
    ...
    {
        _dateTimeOffsetProvider = new SystemDateTimeOffsetProvider();
        InitializeQueues();
    }


    /// <summary>
    /// Use the provided DateTimeOffset Provider
    /// </summary>
    /// <param name="dateTimeProvider"></param>
    public ConcurrentEngine(IDateTimeOffsetProvider dateTimeProvider)
    {
        _dateTimeOffsetProvider = dateTimeProvider;
        InitializeQueues();
    }
    ...
    public ulong TasksCompleted
    {
        get
        {
            ulong total = 0;
            if (_fastQueue != null)
                total += _fastQueue.TasksCompletedCount;
            if (_slowQueue != null)
                total += _slowQueue.TasksCompletedCount;
            if (_mediumQueue != null)
                total += _mediumQueue.TasksCompletedCount;
            return total;
        }
    }
    ...
    private void InitializeQueues()
    {
        ...
        if (_fastQueue == null)
            _fastQueue = new QueueManager("Fast Job Queue", MaxThreadsFast);
        if (_mediumQueue == null)
            _mediumQueue = new QueueManager("Medium Job Queue", MaxThreadsMedium);
        if (_slowQueue == null)
            _slowQueue = new QueueManager("Slow Job Queue", MaxThreadsSlow);
    }
    ...
    public void Start()
    {
        if (Status != EnumConcurrentEngineStatus.Stopped)
            return;
    ...
    public void Execute()
    {
        while (_continueRunning)
        {
            ...
            {
                if (jobPair.Value.IsTimeToRun(_dateTimeOffsetProvider))
                {
                    jobPair.Value.Execute();
                    jobPair.Value.SetNextRunTime(_dateTimeOffsetProvider);
                }
            }
    ...
    public void AddJob(PeriodicJob periodicJob) { Jobs.Add(periodicJob.Id, periodicJob); }
                          ...
                          string checkInterval)
    {
        ...
        PeriodicJob job = new PeriodicJob(name,
                                          methodToRun,
                                          dayTimeInterval,
                                          timeCheckInterval,
                                          AddTask);
    ...
    {
        if (processingTask.TaskSpeed == EnumProcessingTaskSpeed.Fast)
            _fastQueue.AddTask(processingTask);
